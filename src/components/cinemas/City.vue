    ...
    <div>
        <van-index-bar :index-list="computedList" @select="keychange">
          <div  v-for="item in citylist" :key="item.type">
            ...
            <van-cell v-for="city in item.list" :key="city.cityId" :title="city.name"  @click="cityClick(city)"/>
          </div>
...
export default {
  ...
  methods: {
    ...mapMutations(['TabShow', 'TabHidden', 'clearCinemaList', 'changeCityName', 'changeCityId']),
    keychange (data) {
    ...
    renderCity (list) {
      ...
      falist.forEach(faitem => {
        const newlist = list.filter(item =>
          item.pinyin.substring(0, 1).toUpperCase() === faitem
        )
